                                ...
                                <tr>
                                    <td>
                                        <p style="margin:0 0 8px 0;color:#2c3e50;font-size:14px;line-height:1.6;">
                                            <strong>📅 时间范围:</strong> {time_range.get('start', 'N/A')} 至 {time_range.get('end', 'N/A')}
                                        </p>
                                        <p style="margin:0 0 8px 0;color:#2c3e50;font-size:14px;line-height:1.6;">
                    ...
                    <tr>
                        <td style="padding:0 30px;">
                            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                                <tr>
            <div class="stat-card">
                <div class="number">{runs_stats.get('total', 0)}</div>
                <div class="label">总运行数</div>
            </div>
            <div class="stat-card success">
                <div class="number">{runs_stats.get('completed', 0)}</div>
                <div class="label">已完成</div>
            </div>
            <div class="stat-card warning">
                <div class="number">{runs_stats.get('running', 0)}</div>
                <div class="label">运行中</div>
            </div>
        </div>
        
        <h3 style="margin-top: 20px; margin-bottom: 10px;">最近运行</h3>
        <table>
            <thead>
                <tr>
                    <th>运行 ID</th>
...
"""
        ...
        for run_id, run_info in list(runs_detail.items())[-20:]:
            ...
            start = run_info.get('start', 'N/A')
            complete = run_info.get('complete', 'N/A')
...
"""
        ...
        for session_id, session_info in list(sessions_detail.items())[-20:]:
            ...
            start = session_info.get('start', 'N/A')
...
"""
    ...
    <section>
        ...
        <h3 style="margin-top: 20px; margin-bottom: 10px;">常见错误</h3>
"""
        
        if top_errors:
            for error_msg, count in sorted(top_errors.items(), key=lambda x: x[1], reverse=True)[:10]:
                html += f'<div class="error-item"><strong>({count}次)</strong> {error_msg}</div>'
        else:
            html += '<p style="color: #999;">未发现错误或警告</p>'
...
"""
    ...
    <section>
        <h2>📋 最近事件</h2>
        <h3 style="margin-top: 20px; margin-bottom: 10px;">最后20个事件</h3>
"""
        ...
        <div class="event-item">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="background: #E0E0E0; padding: 2px 8px; border-radius: 3px; font-size: 12px;">{event_type}</span>
            </div>
            <div class="event-message"><strong>{subsystem}</strong>: {message}</div>
        </div>
"""
...
"""
    ...
    def _create_footer(self) -> str:
        ...
        return """
    <footer>
        <p>本报告由 OpenClawMonitor 自动生成</p>
        <p>© 2026 OpenClaw 系统监控</p>
    </footer>
</div>
</body>
</html>
